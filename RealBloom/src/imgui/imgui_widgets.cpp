// NOTE:
// Some parts of this file are modified and customized for RealBloom, all of which
// can be found in regions named "RealBloom modifications".
...
#pragma region RealBloom modifications
        ...
        {
            // Use a CmImage to display the colors correctly
            uint32_t cmimage_width = (uint32_t)floorf(sv_picker_size);
            uint32_t cmimage_height = cmimage_width;
            if (color_picker_sv_sqaure_cmimage.get() == nullptr)
            {
                color_picker_sv_sqaure_cmimage = std::make_shared<CmImage>("", "", cmimage_width, cmimage_height, std::array<float, 4>{ 0, 0, 0, 1 }, false, false);
            }
            else
            {
                color_picker_sv_sqaure_cmimage->resize(cmimage_width, cmimage_height, true);
            }
            ...
            // Draw the SV square
            {
                ...
                {
                    for (uint32_t x = 0; x < cmimage_width; x++)
                    {
                        uint32_t red_index = (y * cmimage_width + x) * 4;

                        float u = ((float)x + 0.5f) / (float)cmimage_width;
                        float v = ((float)y + 0.5f) / (float)cmimage_height;

                        float mul = 1.0f - v;

                        buffer[red_index + 0] = lerp(1.0f, hue_color_f.x, u) * mul;
                        buffer[red_index + 1] = lerp(1.0f, hue_color_f.y, u) * mul;
                        buffer[red_index + 2] = lerp(1.0f, hue_color_f.z, u) * mul;
                    }
                }
            }
            color_picker_sv_sqaure_cmimage->moveToGPU();

            // Add the image to the draw list
            draw_list->AddImage(
                (void*)(intptr_t)(color_picker_sv_sqaure_cmimage->getGlTexture()),
                picker_pos,
...
#pragma region RealBloom modifications
        ...
        {
            // Use a CmImage to display the colors correctly
            uint32_t cmimage_width = 8;
            uint32_t cmimage_height = (uint32_t)floorf(sv_picker_size);
            if (color_picker_hue_bar_cmimage.get() == nullptr)
            {
                color_picker_hue_bar_cmimage = std::make_shared<CmImage>("", "", cmimage_width, cmimage_height, std::array<float, 4>{ 0, 0, 0, 1 }, false, false);
            }
            else
            {
                color_picker_hue_bar_cmimage->resize(cmimage_width, cmimage_height, true);
            }
            ...
            // Draw the hue bar
            {
                ...
                for (uint32_t y = 0; y < cmimage_height; y++)
                {
                    for (uint32_t x = 0; x < cmimage_width; x++)
                    {
                        ...
                        static const float hue_colors_r[7]{ 1, 1, 0, 0, 0, 1, 1 };
                        static const float hue_colors_g[7]{ 0, 1, 1, 1, 0, 0, 0 };
                        static const float hue_colors_b[7]{ 0, 0, 0, 1, 1, 1, 0 };

                        float hue_color_index_f = v * 6;
                        uint32_t hue_color_index = (uint32_t)floorf(hue_color_index_f);
                        float hue_color_index_offset = hue_color_index_f - (float)hue_color_index;

                        if (hue_color_index >= 6)
                        {
                            buffer[red_index + 0] = hue_colors_r[6];
                            buffer[red_index + 1] = hue_colors_g[6];
                            buffer[red_index + 2] = hue_colors_b[6];
                        }
                        else
                        {
                            buffer[red_index + 0] = lerp(hue_colors_r[hue_color_index], hue_colors_r[hue_color_index + 1], hue_color_index_offset);
                            buffer[red_index + 1] = lerp(hue_colors_g[hue_color_index], hue_colors_g[hue_color_index + 1], hue_color_index_offset);
                            buffer[red_index + 2] = lerp(hue_colors_b[hue_color_index], hue_colors_b[hue_color_index + 1], hue_color_index_offset);
                        }
                    }
                }
            }
            color_picker_hue_bar_cmimage->moveToGPU();

            // Add the image to the draw list
            draw_list->AddImage(
                (void*)(intptr_t)(color_picker_hue_bar_cmimage->getGlTexture()),
...
#pragma region RealBloom modifications
    if (OCIO_COLOR_CORRECTION)
    {
        std::array<float, 3> rgb = CMS::getDisplayColor(std::array<float, 3>{ R, G, B });
        ImU32 rgb32 = ColorConvertFloat4ToU32(ImVec4(rgb[0], rgb[1], rgb[2], style.Alpha));
        draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, rgb32, sv_cursor_segments);
    }
    else
    {
...
#pragma region RealBloom modifications
void imgui_widgets_cleanup_cmimages()
{
    color_picker_sv_sqaure_cmimage = nullptr;
    color_picker_hue_bar_cmimage = nullptr;
}
#pragma endregion
